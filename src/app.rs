...
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(mut cache) = file_cache.lock() {
        ...
        tracing::error!("failed to lock file_cache for load_from_redb");
    }
    info!("Loaded file cache from redb");
    info!("About to scan_dir");
    std::io::stdout().flush()?;
    if let Ok(mut cache) = file_cache.lock() {
        let scan_span = info_span!("scan_dir");
        let _scan_enter = scan_span.enter();
        cache.scan_dir(watch_root);
        info!(file_count = cache.all_files().count(), "After scan_dir");
    } else {
        tracing::error!("failed to lock file_cache for scan_dir");
    }
    std::io::stdout().flush()?;
    info!("About to start watcher");
    std::io::stdout().flush()?;
    ...
    if let Ok(cache) = file_cache.lock() {
        ...
        info!(count, total_size, "Initial files cached");
    } else {
        tracing::error!("failed to lock file_cache for stats");
    }
